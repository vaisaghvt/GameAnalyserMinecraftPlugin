...
public abstract class DBSynchDataGetSet {
	...
	public static boolean updateLocation(String UUID, Long timeStep, Integer x,
			Integer y, Integer z) {
		List<String> vars = new ArrayList<String>();
		vars.add(UUID);
		vars.add(timeStep.toString());
		vars.add(x.toString());
		vars.add(y.toString());
		vars.add(z.toString());
		return StatisticianPlugin.getInstance().getDB().callStoredProcedure("addNewLocation", vars);
